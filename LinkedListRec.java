...
public class LinkedListRec< E> {
    ...
    /**
     * Removes all duplicated elements in the linked list.
     *
     * @param head The head of the current list
    ...
    private boolean remove(Node< E> head, Node< E> pred, E outData) {
        if (head == null) // Base case – empty list.
        {
            ...
            remove(pred.next, pred, outData); //burada head yollamak yerine pred yolladım. 170.satırda head.next ı atamıstım.
        } else {
            remove(head.next, head, outData);

        }
        return true; // returnlerı if els statement icine yazmadım. Cunku lıstenın sonuna kadar bakmasını ıstıyorum.
    }
    ...
    public boolean remove(E outData) {
        if (head == null) {
            ...
            head = head.next;
            remove(head, head, outData); // burada remove u cagırdımm. Cunku ilk eleman eoutDataya esıtse dırek cıkıyordu.
       //head yollamamın nedenı zaten 190.satırda head e head ın nextını yollamam
        } else {
            remove(head.next, head, outData);
        }
        return false;
